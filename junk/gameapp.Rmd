...
df_comb_lp_plot %>% 
  ggplot(aes(x = poms, y = measures, color = Class))+
  geom_jitter(alpha = 0.3, width = 0.1, height = 0.3)+
...
ggsave(filename = "LPA_GGPLOT2.png", width = 10, height = 5)
...
ggsave(filename = "grade_class.png", width = 8, height = 5)
```
...
psych::alpha(survey1_to_fa %>% select(starts_with("AMG")) )
```
...
mfin.model <- 'AMG =~ AMG_1 + AMG_2 + AMG_3 + AMG_4
   ...
   TM =~ TM_1 + TM_2 + TM_3 + TM_4 + TM_5 + TM_6 + TM_7_reversed + TM_8_reversed'
...
mfin.model <- 'Selfefficacy =~ Selfefficacy_0 + Selfefficacy_2 + Selfefficacy_3 + Selfefficacy_4 + Selfefficacy_5 + Selfefficacy_6 + Selfefficacy_7 + Selfefficacy_8
   Selfeval =~ Selfeval_1 + selfeval_2 + selfeval_3 + Selfeval_4 + Selfeval_5 + Selfeval_6 + GSL_1 + GSL_2  + GSL_4
   # GSL =~ GSL_1 + GSL_2 + GSL_3 + GSL_4 + GSL_5
...
by the majority of modern SEM guidebooks: CFI (Comparative Fit Index), acceptable CFI values ≥ 0.90; 
RMSEA (Root Mean Square Error of Approximation), 
acceptable RMSEA values ≤ 0.05; and SRMR
(Standardized Root Mean Square Residual), acceptable SRMR values ≤ 0.08. 
...
of observations [Nasledov 2012: 348–353]. A comparison of f
